...
GP_TRAINER="${GP_TRAINER:-prom-gp-trainer_std.py}"
...
if [[ ! -f "$CLUSTER_DIR/$GP_TRAINER" ]]; then
  echo "ERROR: Missing trainer script: $CLUSTER_DIR/$GP_TRAINER"
  echo "Available options:"
  ls -1 "$CLUSTER_DIR"/prom-gp-trainer*.py 2>/dev/null || true
  exit 1
fi
